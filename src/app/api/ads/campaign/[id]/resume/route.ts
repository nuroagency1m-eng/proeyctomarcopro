...
export async function POST(_req: Request, { params }: { params: { id: string } }) {
    ...
    try {
        ...
        await adapter.resumeCampaign(accessToken, campaign.connectedAccount.providerAccountId, campaign.providerCampaignId)
        ...
        console.error('[ResumeCampaign]', err)
        return NextResponse.json({ error: err.message || 'Error al reanudar la campaña' }, { status: 500 })
    }
}
